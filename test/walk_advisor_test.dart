...
void main() {
  group('WalkAdvisor.weakestTrait', () {
    ...
    test(
      'an empty profile is allowed (all zero) and returns a valid trait',
      () {
  ...
  group('WalkAdvisor.gentleSuggestion', () {
    test('suggests the weakest trait and a matching place, safely worded', () {
      ...
      // Never pushy: framed as optional, no speed/distance prompts.
      expect(s.message, contains('ついで'));
  ...
  group('WalkAdvisor.moodForecast', () {
    test('describes the dominant trait of a planned route', () {
      ...
      expect(forecast, contains('静けさ'));
      expect(forecast, contains('ほっとしている'));
    });
